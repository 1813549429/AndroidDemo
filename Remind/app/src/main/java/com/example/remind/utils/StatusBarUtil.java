...
public class StatusBarUtil {
    ...
    public static void setStatusBarGone(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            int count = decorView.getChildCount();
            if (count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
                StatusBarView barView = (StatusBarView) decorView.getChildAt(count - 1);
                barView.setVisibility(View.GONE);
            }
        }
    }

    public static void setStatusBarVisible(Activity activity) {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT && Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
            ViewGroup decorView = (ViewGroup) activity.getWindow().getDecorView();
            int count = decorView.getChildCount();
            if (count > 0 && decorView.getChildAt(count - 1) instanceof StatusBarView) {
     ...
     * @param isVisible
     */
    public static void hideStatusBar(Activity activity, boolean isVisible) {
        if(activity==null){
            return;
        }
        if (isVisible) {
...
//            lp.flags |= View.SYSTEM_UI_FLAG_HIDE_NAVIGATION;
